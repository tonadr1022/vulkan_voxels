 ...
 * @version 4.1.0
 * @date 2024-03-22
 * @copyright Copyright (c) 2024 Barak Shoshany. Licensed under the MIT license. If you found this
 ...
 * doi:10.1016/j.softx.2024.101687, SoftwareX 26 (2024) 101687, arXiv:2105.00613
 *
 * @brief BS::thread_pool: a fast, lightweight, and easy-to-use C++17 thread pool library. This
 * header file contains the main thread pool class and some additional classes and definitions. No
 * other files are needed in order to use the thread pool itself.
 */
...
/**
 * @brief A convenient shorthand for the type of `std::thread::hardware_concurrency()`. Should
 * evaluate to unsigned int.
 */
using concurrency_t = std::invoke_result_t<decltype(std::thread::hardware_concurrency)>;

#ifdef BS_THREAD_POOL_ENABLE_PRIORITY
/**
 * @brief A type used to indicate the priority of a task. Defined to be an integer with a width of
 * (at least) 16 bits.
 */
...
/**
 * @brief A type returned by `BS::this_thread::get_index()` which can optionally contain the index
 * of a thread, if that thread belongs to a `BS::thread_pool`. Otherwise, it will contain no value.
 */
using optional_index = std::optional<size_t>;

/**
 * @brief A type returned by `BS::this_thread::get_pool()` which can optionally contain the pointer
 * to the pool that owns a thread, if that thread belongs to a `BS::thread_pool`. Otherwise, it will
 * contain no value.
 */
...
class [[nodiscard]] thread_info_index {
 ...
 public:
   ...
   * object, it will have an index from 0 to `BS::thread_pool::get_thread_count() - 1`. Otherwise,
   * for example if this thread is the main thread or an independent `std::thread`, `std::nullopt`
   * will be returned.
   *
   * @return An `std::optional` object, optionally containing a thread index. Unless you are 100%
   * sure this thread is in a pool, first use `std::optional::has_value()` to check if it contains a
   * value, and if so, use `std::optional::value()` to obtain that value.
   */
...
class [[nodiscard]] thread_info_pool {
 ...
 public:
  /**
   * @brief Get the pointer to the thread pool that owns the current thread. If this thread belongs
   * to a `BS::thread_pool` object, a pointer to that object will be returned. Otherwise, for
   * example if this thread is the main thread or an independent `std::thread`, `std::nullopt` will
   * be returned.
   *
   * @return An `std::optional` object, optionally containing a pointer to a thread pool. Unless you
   * are 100% sure this thread is in a pool, first use `std::optional::has_value()` to check if it
   * contains a value, and if so, use `std::optional::value()` to obtain that value.
   */
...
/**
 * @brief A `thread_local` object used to obtain information about the thread pool that owns the
 * current thread.
 */
...
class [[nodiscard]] multi_future : public std::vector<std::future<T>> {
 public:
  ...
  // The copy constructor and copy assignment operator are deleted. The elements stored in a
  // `multi_future` are futures, which cannot be copied.
  ...
  /**
   * @brief Get the results from all the futures stored in this `multi_future`, rethrowing any
   * stored exceptions.
   *
  ...
  /**
   * @brief Wait for all the futures stored in this `multi_future`, but stop waiting after the
  ...
  /**
   * @brief Wait for all the futures stored in this `multi_future`, but stop waiting after the
   * specified time point has been reached. This function first waits for the first future until the
   * desired time point. If that future is ready before the time point is reached, this function
   * waits for the second future until the desired time point. It continues similarly until the time
   * point is reached.
   *
...
class [[nodiscard]] thread_pool {
 public:
  ...
  /**
   * @brief Construct a new thread pool. The number of threads will be the total number of hardware
   * threads available, as reported by the implementation. This is usually determined by the number
   * of cores in the CPU. If a core is hyperthreaded, it will count as two threads.
   */
   ...
   *
   * @param init_task An initialization function to run in each thread before it starts to execute
   * any submitted tasks. The function must take no arguments and have no return value. It will only
   * be executed exactly once, when the thread is first constructed.
   */
  ...
  // The copy and move constructors and assignment operators are deleted. The thread pool uses a
  // mutex, which cannot be copied or moved.
   ...
   * Note that if the pool is paused, then any tasks still in the queue will never be executed.
   */
  ~thread_pool() {
    wait();
    destroy_threads();
  ...
  /**
   * @brief Get a vector containing the underlying implementation-defined thread handles for each of
   * the pool's threads, as obtained by `std::thread::native_handle()`. Only enabled if
  ...
  /**
   * @brief Get the total number of unfinished tasks: either still waiting in the queue, or running
   * in a thread. Note that `get_tasks_total() == get_tasks_queued() + get_tasks_running()`.
   *
   ...
   * although any tasks already executed will keep running until they are finished. Only enabled if
   * `BS_THREAD_POOL_ENABLE_PAUSE` is defined.
   */
  void pause() {
    const std::scoped_lock tasks_lock(tasks_mutex);
   ...
   * specified priority. To push a function with arguments, enclose it in a lambda expression. Does
   ...
   * @param priority The priority of the task. Should be between -32,768 and 32,767 (a signed 16-bit
   * integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   */
  template <typename F>
   ...
   * the start and end of the block, so that it is only called only once per block, but it is up to
   * the user make sure the block function correctly deals with all the indices in each block. Does
   * not return a `multi_future`, so the user must use `wait()` or some other method to ensure that
   ...
   * `first_index` to `(index_after_last - 1)` inclusive. In other words, it will be equivalent to
   * `for (T i = first_index; i < index_after_last; ++i)`. Note that if `index_after_last <=
   * first_index`, no blocks will be submitted.
   * @param block A function that will be called once per block. Should take exactly two arguments:
   * the first index in the block and the index after the last index in the block. `block(start,
   * end)` should typically involve a loop of the form `for (T i = start; i < end; ++i)`.
   * @param num_blocks The maximum number of blocks to split the loop into. The default is 0, which
   * means the number of blocks will be equal to the number of threads in the pool.
   * @param priority The priority of the tasks. Should be between -32,768 and 32,767 (a signed
   * 16-bit integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   */
  ...
  void detach_blocks(const T first_index, const T index_after_last, F&& block,
    ...
    if (index_after_last > first_index) {
      ...
      for (size_t blk = 0; blk < blks.get_num_blocks(); ++blk)
        detach_task([block = std::forward<F>(block), start = blks.start(blk), end = blks.end(blk)] {
          block(start, end);
  ...
  /**
   * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block
   * separately to the queue, with the specified priority. The loop function takes one argument, the
   * loop index, so that it is called many times per block. Does not return a `multi_future`, so the
   * user must use `wait()` or some other method to ensure that the loop finishes executing,
   ...
   * `for (T i = first_index; i < index_after_last; ++i)`. Note that if `index_after_last <=
   * first_index`, no blocks will be submitted.
   * @param loop The function to loop through. Will be called once per index, many times per block.
   * Should take exactly one argument: the loop index.
   * @param num_blocks The maximum number of blocks to split the loop into. The default is 0, which
   * means the number of blocks will be equal to the number of threads in the pool.
   * @param priority The priority of the tasks. Should be between -32,768 and 32,767 (a signed
   * 16-bit integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
  ...
  void detach_loop(const T first_index, const T index_after_last, F&& loop,
    ...
    if (index_after_last > first_index) {
      const blocks blks(first_index, index_after_last, num_blocks ? num_blocks : thread_count);
      for (size_t blk = 0; blk < blks.get_num_blocks(); ++blk)
        detach_task([loop = std::forward<F>(loop), start = blks.start(blk), end = blks.end(blk)] {
   ...
   * priority. Does not return a `multi_future`, so the user must use `wait()` or some other method
   * to ensure that the sequence finishes executing, otherwise bad things will happen.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   ...
   * @param sequence The function used to define the sequence. Will be called once per index. Should
   * take exactly one argument, the index.
   * @param priority The priority of the tasks. Should be between -32,768 and 32,767 (a signed
   * 16-bit integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   */
  template <typename T, typename F>
   ...
   * implementation. Waits for all currently running tasks to be completed, then destroys all
   * threads in the pool and creates a new thread pool with the new number of threads. Any tasks
   * that were waiting in the queue before the pool was reset will then be executed by the new
   * threads. If the pool was paused before resetting it, the new pool will be paused as well.
   */
  void reset() {
    reset(0, [] {});
  }
   ...
   * completed, then destroys all threads in the pool and creates a new thread pool with the new
   * number of threads and initialization function. Any tasks that were waiting in the queue before
   * the pool was reset will then be executed by the new threads. If the pool was paused before
  ...
  /**
   * @brief Reset the pool with a new number of threads and a new initialization function. Waits for
   * all currently running tasks to be completed, then destroys all threads in the pool and creates
   * a new thread pool with the new number of threads and initialization function. Any tasks that
   * were waiting in the queue before the pool was reset will then be executed by the new threads.
  ...
  /**
   * @brief Submit a function with no arguments into the task queue, with the specified priority. To
   * submit a function with arguments, enclose it in a lambda expression. If the function has a
   ...
   * separately to the queue, with the specified priority. The block function takes two arguments,
   * the start and end of the block, so that it is only called only once per block, but it is up to
   * the user make sure the block function correctly deals with all the indices in each block.
   ...
   * `first_index` to `(index_after_last - 1)` inclusive. In other words, it will be equivalent to
   * `for (T i = first_index; i < index_after_last; ++i)`. Note that if `index_after_last <=
   * first_index`, no blocks will be submitted, and an empty `multi_future` will be returned.
   * @param block A function that will be called once per block. Should take exactly two arguments:
   * the first index in the block and the index after the last index in the block. `block(start,
   * end)` should typically involve a loop of the form `for (T i = start; i < end; ++i)`.
   * @param num_blocks The maximum number of blocks to split the loop into. The default is 0, which
   ...
   * @param priority The priority of the tasks. Should be between -32,768 and 32,767 (a signed
   * 16-bit integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   * @return A `multi_future` that can be used to wait for all the blocks to finish. If the block
   * function returns a value, the `multi_future` can also be used to obtain the values returned by
   * each block.
   */
   ...
   * `first_index` to `(index_after_last - 1)` inclusive. In other words, it will be equivalent to
   * `for (T i = first_index; i < index_after_last; ++i)`. Note that if `index_after_last <=
   * first_index`, no tasks will be submitted, and an empty `multi_future` will be returned.
   * @param loop The function to loop through. Will be called once per index, many times per block.
   * Should take exactly one argument: the loop index. It cannot have a return value.
   * @param num_blocks The maximum number of blocks to split the loop into. The default is 0, which
   * means the number of blocks will be equal to the number of threads in the pool.
   * @param priority The priority of the tasks. Should be between -32,768 and 32,767 (a signed
   * 16-bit integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
  ...
  /**
   * @brief Submit a sequence of tasks enumerated by indices to the queue, with the specified
   * priority. Returns a `multi_future` that contains the futures for all of the tasks.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   * @tparam F The type of the function used to define the sequence.
   ...
   * iterate from `first_index` to `(index_after_last - 1)` inclusive. In other words, it will be
   * equivalent to `for (T i = first_index; i < index_after_last; ++i)`. Note that if
   * `index_after_last <= first_index`, no tasks will be submitted, and an empty `multi_future` will
   * be returned.
   * @param sequence The function used to define the sequence. Will be called once per index. Should
   * take exactly one argument, the index.
   * @param priority The priority of the tasks. Should be between -32,768 and 32,767 (a signed
   * 16-bit integer). The default is 0. Only enabled if `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   * @return A `multi_future` that can be used to wait for all the tasks to finish. If the sequence
   * function returns a value, the `multi_future` can also be used to obtain the values returned by
   * each task.
   */
  template <typename T, typename F, typename R = std::invoke_result_t<std::decay_t<F>, T>>
  [[nodiscard]] multi_future<R> submit_sequence(const T first_index, const T index_after_last,
                                                F&& sequence BS_THREAD_POOL_PRIORITY_INPUT) {
    if (index_after_last > first_index) {
      multi_future<R> future;
   ...
   * enabled if `BS_THREAD_POOL_ENABLE_PAUSE` is defined.
   */
  void unpause() {
    {
      const std::scoped_lock tasks_lock(tasks_mutex);
...
#define BS_THREAD_POOL_PAUSED_OR_EMPTY tasks.empty()
   ...
   * However, if the pool is paused, this function only waits for the currently running tasks
   * (otherwise it would wait forever). Note: To wait for just one specific task, use
   * `submit_task()` instead, and call the `wait()` member function of the generated future.
   *
   * @throws `wait_deadlock` if called from within a thread of the same pool, which would result in
   * a deadlock. Only enabled if `BS_THREAD_POOL_ENABLE_WAIT_DEADLOCK_CHECK` is defined.
   */
  void wait() {
#ifdef BS_THREAD_POOL_ENABLE_WAIT_DEADLOCK_CHECK
    ...
    tasks_done_cv.wait(tasks_lock,
                       [this] { return (tasks_running == 0) && BS_THREAD_POOL_PAUSED_OR_EMPTY; });
    waiting = false;
  }
  ...
  /**
   * @brief An exception that will be thrown by `wait()`, `wait_for()`, and `wait_until()` if the
   * user tries to call them from within a thread of the same pool, which would result in a
   * deadlock.
   */
 ...
 private:
   ...
   *
   * @param num_threads The parameter passed to the constructor or `reset()`. If the parameter is a
   * positive number, then the pool will be created with this number of threads. If the parameter is
   * non-positive, or a parameter was not supplied (in which case it will have the default value of
   * 0), then the pool will be created with the total number of hardware threads available, as
   * obtained from `std::thread::hardware_concurrency()`. If the latter returns zero for some
   * reason, then the pool will be created with just one thread.
   * @return The number of threads to use for constructing the pool.
   */
  [[nodiscard]] static concurrency_t determine_thread_count(const concurrency_t num_threads) {
    if (num_threads > 0) return num_threads;
    if (std::thread::hardware_concurrency() > 0) return std::thread::hardware_concurrency();
    return 1;
  }

  /**
   * @brief A worker function to be assigned to each thread in the pool. Waits until it is notified
   * by `detach_task()` that a task is available, and then retrieves the task from the queue and
   * executes it. Once the task finishes, the worker notifies `wait()` in case it is waiting.
   *
   * @param idx The index of this thread.
   * @param init_task An initialization function to run in this thread before it starts to execute
   * any submitted tasks.
   */
  void worker(const concurrency_t idx, const std::function<void()>& init_task) {
    ...
    while (true) {
      ...
      task_available_cv.wait(
          tasks_lock, [this] { return !BS_THREAD_POOL_PAUSED_OR_EMPTY || !workers_running; });
  ...
  class [[nodiscard]] pr_task {
   ...
   public:
     ...
     */
    explicit pr_task(const std::function<void()>& task_, const priority_t priority_ = 0)
     ...
     * @param priority_ The desired priority.
     */
    explicit pr_task(std::function<void()>&& task_, const priority_t priority_ = 0)
   ...
   * temporarily stop retrieving new tasks out of the queue, although any tasks already executed
  ...
  /**
   * @brief A flag indicating that `wait()` is active and expects to be notified whenever a task is
   * done.
   */
  bool waiting = false;
